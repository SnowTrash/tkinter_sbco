...
## Cambiando el WIN LOGIN por CustomTkinter objects
...
class Login():
    ...
    def init_page(self):
        ...
        self.page = Frame(self.root)  # Creamos el root(raíz) Frame 
        self.page.pack()
        ...
        Label(self.page, text="Contraseña: ").grid(row=2, stick="W", pady=10)
        ...
        button_cancel = Button(self.page, text="Salir", command=self.doCancel)
    ...
    def init_menu(self):
        ...
        menubar.add_cascade(label="Archivo/file", menu=filemenu)
    ...
    def doLogin(self):
        ...
        if res is True:
            # if username == "admin" and password == "admin": # Probando con variables locales
            self.page.destroy()

            glv.set_variable("CURRENT_USER_NAME", str(username))
            MainPage(self.root)
        else:
            messagebox.showinfo(title="Error", message="¡Error en la cuenta o constraseña！")
